...
import UpdateProductionView from "@/components/floor_consumption/UpdateProductionView";
...
export default function UpdateConsumption() {
  ...
  function findConsumption(products) {
    products.forEach((product, index) => {
      const batch = product.batch.find((b) => b.date.toString() === date);
      const carton = product.carton.find((b) => b.date.toString() === date);

      const batchExist = batch && Number(batch.qty) > 0;
      const cartonExist = carton && Number(carton.qty) > 0;

      if (batchExist || cartonExist) {
        existAddConsumption(product, Number(batch.qty), Number(carton.qty));
      }
    });
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Find existing consumption data
        findConsumption(recipeData);
  ...
  function existAddConsumption(product, batch, carton) {
    ...
    setConsumption((prev) => [
      ...prev,
      {
        ...
        rm: (product?.rm || []).map((i) => ({
          id: i.id,
          qty: ((parseFloat(i.qty) || 0) * batchNum).toFixed(2),
          unit: i.unit,
        })),
        pm: (product?.carton_pm || []).map((i) => ({
          id: i.id,
          qty: ((parseFloat(i.qty) || 0) * cartonNum).toFixed(2),
          unit: i.unit,
        })),
      },
    ]);
      ...
      ...prev,
      {
        ...
        rm:
          batchNum <= 0
            ...
            : (product?.rm || []).map((i) => ({
                id: i.id,
                qty: String((parseFloat(i.qty) || 0) * batchNum),
                unit: i.unit,
              })),
        pm:
          cartonNum <= 0
            ...
            : (product?.carton_pm || []).map((i) => ({
                id: i.id,
                qty: String((parseFloat(i.qty) || 0) * cartonNum),
                unit: i.unit,
              })),
      },
    ]);
  ...
  function addConsumption() {
    ...
    if (!batch || !carton) {
      const confirmMessage = `${
        !batch ? "Batch" : "Carton"
      } input is empty. Are you sure you only want to add ${
        !batch ? "Batch" : "Carton"
      } consumption?`;
      ...
      <div className="max-w-7xl mx-auto sm:p-6 lg:p-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">
            Update consumption data for {selectedDate} {user?.current_period}
          </p>
        </div>
