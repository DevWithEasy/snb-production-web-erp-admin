...
export default function MaterialsDetails() {
  ...
  if (!type) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        ...
        <p className="text-gray-600 text-center">Loading parameters...</p>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mb-4"></div>
        <p className="text-gray-600 text-center">Loading material details...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto  sm:p-4 lg:p-6">
        ...
        {/* Summary Section */}
        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
          <h2 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-gray-800 border-b border-gray-200 pb-2 sm:pb-3">
            Summary
          </h2>
          
          <div className="space-y-2 sm:space-y-3">
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              <span className="text-gray-700 font-medium text-sm sm:text-base">Opening:</span>
              <span className="text-gray-900 font-semibold text-sm sm:text-base">{itemData?.opening || 0}</span>
            </div>
            
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              <span className="text-gray-700 font-medium text-sm sm:text-base">Total Received:</span>
              <span className="text-gray-900 font-semibold text-sm sm:text-base">{formatNumber(total_received)}</span>
            </div>
            
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              <span className="text-gray-700 font-medium text-sm sm:text-base">Total Consumption:</span>
              <span className="text-gray-900 font-semibold text-sm sm:text-base">{formatNumber(total_consumption)}</span>
            </div>
            
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              <span className="text-gray-700 font-medium text-sm sm:text-base">Floor Stock:</span>
              <span className="text-blue-600 font-bold text-sm sm:text-base">{formatNumber(Stock)}</span>
            </div>

            {itemData?.closing !== null && (
              <div>
                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                  <span className="text-gray-700 font-medium text-sm sm:text-base">Need Adjustment:</span>
                  <span 
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
              ...
              <tbody>
                {allDates.map((date, index) => {
                  ...
                  return (
                    <tr 
                      key={date}
                      className={`border-b border-gray-200 hover:bg-gray-50 cursor-pointer active:bg-blue-50 transition-colors ${
                    ...
                    >
                      <td className="p-2 sm:p-3 text-gray-700 text-sm sm:text-base">
                        <div className="flex items-center space-x-2">
                          <span>Date {date}</span>
                          {isMobile && (
                            <FaEdit className="text-blue-500 text-xs opacity-0 group-hover:opacity-100" />
                          )}
                        </div>
                      </td>
                      <td className="p-2 sm:p-3 text-center text-gray-600 text-sm sm:text-base">
          ...
          {allDates.length === 0 && (
            <div className="text-center py-6 sm:py-8">
              <FaBox className="text-gray-300 text-3xl sm:text-4xl mx-auto mb-2" />
              <p className="text-gray-500 text-sm sm:text-base">No daily data available</p>
            </div>
          )}
        </div>
        ...
        {/* Edit Modal */}
        {modalVisible && (
          <div className="fixed inset-0 bg-gray-500/50 bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4">
            <div className="bg-white rounded-xl p-4 sm:p-6 w-full max-w-md mx-auto max-h-[90vh] overflow-y-auto">
              <h2 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-gray-800 text-center">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 sm:py-3 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full border rounded-lg px-3 py-2 sm:py-3 text-sm sm:text-base focus:outline-none focus:ring-2 ${
                      error 
                  ...
                  />
                  {expression && (
                    <p className="text-sm text-green-600 mt-1 sm:mt-2 flex items-center">
                      <FaCalculator className="mr-1" />
                      {expression}
                    </p>
                  )}
                  {error && (
                    <p className="text-sm text-red-600 mt-1 sm:mt-2">Invalid expression</p>
                  )}
              ...
              <div className="flex justify-between items-center mt-4 sm:mt-6 gap-2">
                <button
                  ...
                  disabled={updating}
                  className="px-3 sm:px-4 py-2 text-gray-600 hover:text-gray-800 disabled:opacity-50 transition-colors text-sm sm:text-base flex-1"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 sm:px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm sm:text-base flex-1 flex items-center justify-center"
                >
                  {updating ? (
